...
def generate_device_details(csv_row):
    ...
    for csv_field, dcim_custom_object_attr in device_custom_field_map.items():
        if csv_attr := csv_row.get(csv_field):
            associated_wlc = netbox.dcim.devices.get(name=csv_attr)
            try:
...
def update_interfaces(device_object, csv_row):
    """
    ...
    device_interfaces = netbox.dcim.interfaces.filter(device=device_object)
    ...
    iface_result = netbox.dcim.interfaces.update(interfaces)
...
def create_or_update_device(device_detail_dict):
    ...
    try:
        ...
        device_id = netbox.dcim.devices.get(name=device_detail_dict["name"])

        # Device exists and update is enabled (default):
        if device_id:
            print("YES\n\t\tUpdating device... ", end="")
            device_detail_dict.update({"id": device_id.id})
            nb_result = netbox.dcim.devices.update([device_detail_dict])
            print("OK")

        # No device exists, create
        else:
            print("NO\n\t\tCreating device... ", end="")
            nb_result = netbox.dcim.devices.create([device_detail_dict])
            print("OK")
        ...
        with open(csv_file, "r", encoding="utf-8-sig") as csvfile:
            ...
            for row in reader:
                ...
                if current_device is not None:
                    ...
                    update_interfaces(device_object=current_device, csv_row=row)
